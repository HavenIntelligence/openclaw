...
export function ArchitectureSection() {
  return (
    <>
      ...
      <Paragraph>
        ClawPort is a Next.js 16 dashboard for managing OpenClaw AI agents. It provides an org chart
        (Org Map), direct agent chat with multimodal support, cron monitoring, kanban task board,
        and memory browsing. All AI calls route through the OpenClaw gateway -- no separate API keys
        needed.
      </Paragraph>
  ...
  2. Bundled lib/agents.json            (default)`}
      ...
      <Paragraph>
        ...
        <InlineCode>loadRegistry()</InlineCode>. <InlineCode>lib/agents.ts</InlineCode> calls it to
        build the full agent list (merging in SOUL.md content from the workspace). Users customize
        their agent team by dropping an <InlineCode>agents.json</InlineCode> into their workspace --
  ...
  -> Returns assistant text via SSE`}
      ...
      <InfoCard title="Design Decisions">
        <BulletList
          items={[
            <>
              <strong style={{ color: "var(--text-primary)" }}>Why send-then-poll?</strong>{" "}
              chat.send is async -- it returns immediately. We poll chat.history until the
              assistant's response appears.
            </>,
            <>
              <strong style={{ color: "var(--text-primary)" }}>Why CLI and not WebSocket?</strong>{" "}
              The gateway WebSocket requires device keypair signing for operator.write scope. The
              CLI has the device keys; custom clients don't.
            </>,
            <>
              <strong style={{ color: "var(--text-primary)" }}>Why resize to 1200px?</strong> macOS
              ARG_MAX is 1MB. Unresized photos can produce multi-MB base64 that exceeds CLI argument
              limits (E2BIG error).
            </>,
          ]}
        />
