...
export function ComponentsSection() {
  return (
    <>
      ...
      <Paragraph>
        ClawPort's component tree follows the standard Next.js App Router pattern: providers wrap
        the entire app, layout components handle the sidebar and navigation, and each route renders
    ...
    SettingsProvider (app/settings-provider.tsx)
      ...
      <InfoCard title="ThemeProvider">
        <BulletList
          items={[
            ...
            <>
              Consumer hook: <InlineCode>{"const { theme, setTheme } = useTheme()"}</InlineCode>
            </>,
            <>
              Persists to <InlineCode>localStorage('clawport-theme')</InlineCode>, defaults to
              'dark'
            </>,
          ]}
        />
      ...
      <InfoCard title="SettingsProvider">
        <BulletList
          items={[
            ...
            <>
              Persists to <InlineCode>localStorage('clawport-settings')</InlineCode>
            </>,
          ]}
        />
      </InfoCard>
      ...
      <Table
        ...
        rows={[
          ...
          ["FileAttachment", "File attachment bubble with type-specific icon and download button."],
          ["MediaPreview", "Horizontal strip of staged attachment thumbnails before sending."],
        ]}
      />
      ...
      <Table
        ...
        rows={[
          ...
          ["GridView", "Card-based grid with team grouping hierarchy. Responsive: 1/2/3 columns."],
          ["FeedView", "Activity feed focused on cron status with stat cards and filter pills."],
        ]}
      />
      ...
      <Table
        ...
        rows={[
          ...
          ["TicketDetailPanel", "Slide-in side panel for ticket details and inline agent chat."],
          [
            "CreateTicketModal",
      ...
      <Table
        ...
        rows={[
          ...
          ["DynamicFavicon", "Generates favicon from portal emoji/icon using Canvas API."],
          ["ErrorState", "Full-screen error display with optional retry button."],
          ["Breadcrumbs", "Breadcrumb navigation bar with Lucide ChevronRight separators."],
        ]}
      />

      <SubHeading>UI Primitives</SubHeading>
      <Paragraph>
        Radix-based primitive components in <InlineCode>components/ui/</InlineCode> (shadcn/ui-style
        wrappers):
      </Paragraph>
      <BulletList
        items={["Badge, Button, Card, Dialog, ScrollArea, Separator, Skeleton, Tabs, Tooltip"]}
      />
    </>
  );
}
