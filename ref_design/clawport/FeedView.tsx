...
export function FeedView({ agents, crons, selectedId, onSelect }: FeedViewProps) {
  ...
  const handlePillKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    const next =
      e.key === "ArrowRight" ? (idx + 1) % pills.length : (idx - 1 + pills.length) % pills.length;
    pills[next].focus();
    pills[next].click();
  }, []);
          ...
          icon={
            <svg width="18" height="18" viewBox="0 0 16 16" fill="none">
              <circle
                ...
                cy="8"
                r="6.5"
                stroke={counts.error > 0 ? "var(--system-red)" : "var(--text-tertiary)"}
                strokeWidth="1.2"
              />
              <line
                x1="8"
              ...
              <circle
                ...
                cy="10.5"
                r="0.6"
                fill={counts.error > 0 ? "var(--system-red)" : "var(--text-tertiary)"}
              />
            </svg>
          }
        />
        ...
        >
          ...
          <div style={{ fontSize: "var(--text-caption1)", marginTop: "var(--space-1)" }}>
            {filter !== "all"
              ...
              : "Crons will appear here once configured"}
          </div>
        </div>
      ) : (
        <div
