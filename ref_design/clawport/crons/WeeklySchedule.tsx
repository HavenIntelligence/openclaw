      ...
      >
        ...
        {slot.cron.nextRun && (
          <span style={{ color: "var(--text-tertiary)" }}>
            Next:{" "}
            {new Date(slot.cron.nextRun).toLocaleString([], {
              weekday: "short",
              hour: "numeric",
              minute: "2-digit",
            })}
...
export function WeeklySchedule({ crons }: WeeklyScheduleProps) {
  ...
  const { slotsByDayHour, activeHours } = useMemo(() => {
    ...
    for (const [key, slots] of map) {
      map.set(
        key,
        slots.toSorted((a, b) => a.minute - b.minute || a.cron.name.localeCompare(b.cron.name)),
      );
    }

    // Active hours sorted
    const activeHours = Array.from(hourSet).toSorted((a, b) => a - b);
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            borderLeft: `3px solid ${color}`,
                            transition:
                              "background 150ms var(--ease-smooth), box-shadow 150ms var(--ease-smooth)",
                            boxShadow: isActive
