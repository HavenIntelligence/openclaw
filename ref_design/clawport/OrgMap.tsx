...
function buildEdges(agents: Agent[], selectedId: string | null): Edge[] {
  ...
  if (selectedId) {
    ...
    if (sel) {
      if (sel.reportsTo) {
        selectedAgentIds.add(sel.reportsTo);
      }
  ...
  for (const t of teams) {
    ...
    for (const id of ids) {
      ...
      for (const cid of a.directReports) {
        if (ids.includes(cid)) {
          colEdges.push([id, cid]);
        }
      }
    }
    columns.push({
      label: `Team ${t.manager.name}`,
